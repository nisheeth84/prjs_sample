      ...
      if ($(settings.result).length > 0) {
        ...
        if (settings.multiple) {
          for (var i = 0, files_len = files.length; i < files_len; i++) {
            ...
            var elm = $('<div class="hc-upload"><button type="button" class="hc-del smooth">&times;</button></div>')
              .append(img)
              .append(input);
...
jQuery(function($) {
  ...
  var UI = {
    ...
    slider: function() {
      /*$('.slider-cas').slick({
            	nextArrow: '<img src="images/next.svg" class="next" alt="Next">',
            	prevArrow: '<img src="images/prev.svg" class="prev" alt="Prev">',
            })
  ...
  /*custom here*/
  $('.sidebar-control .expand').click(function(event) {
    $(this)
      .children('i')
  ...
  const button1 = document.querySelector('#open-popup-window');
  button1.addEventListener('click', openWindow);
  function openWindow() {
    // alert($(window).height())
    open('../common/thiet-dinh-dieu-kien-search.html', '_blank', `width=${width},height=${height},left=0,top=0`);
  }
});
