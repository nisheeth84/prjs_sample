...
const ProductNameEl = styled.span`
...
export interface IListContentRowProps {
  ...
  contentCell?: (rowData, field) => JSX.Element; // render element from parent component
...
const ListContentRow: React.FC<IListContentRowProps> = props => {
  ...
  const renderWrapCell = (field, index) => {
    ...
    return (
      <Td
        ...
        data-tag={dataWidth}
      >
        {props.contentCell && props.contentCell(props.record, field)}
      </Td>
    );
  };
  ...
  const productNameSpanStyle = useMemo(() => {
    if (props.modeDisplay === ScreenMode.DISPLAY || !props.isSetting  ) {
      return { width: 600 };
    } else {
      return { width: 300 };
    }
  }, [props.isSetting, props.modeDisplay]);
const trId = `tr_${props.record?.productId}`
          ...
          <>
            ...
            <Td  style={{
              ...
              <div className="product-detail" style={productNameColStyle}>
                <img src={props.record.productImagePath} alt="" className="custom-list-content-row" />
