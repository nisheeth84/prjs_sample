...
import { MENU_TYPE, DND_DEPARTMENT_TYPE, SELECT_TARGET_TYPE, PARTICIPANT_TYPE } from '../constants';
...
const DepartmentCard: React.FC<IDepartmentCardProps> = (props) => {
  ...
  const showDeleteDepartmentDialog = async (sourceDepartment) => {
    const result = await ConfirmDialog({
      ...
      message: translate('messages.WAR_COM_0001', { itemName: sourceDepartment.departmentName }),
  ...
  const SubMenuDepartment = () => {
    return (
      <div className="box-select-option version2" ref={subMenuRef} style={{ left: setPosition().left, top: setPosition().top }}>
        <ul>
          <li onClick={() => { props.dirtyCheck(() => { setIsShowSubMenu(false); onClickOpenDepartmentPopup(props.sourceDepartment.departmentId) }) }}>
            {translate('employees.department-card.submenu.edit-department')}
          </li>
          <li onClick={() => { props.dirtyCheck(() => { setIsShowSubMenu(false); showDeleteDepartmentDialog(props.sourceDepartment) }) }}>
  ...
  const actionClickDeparment = () => {
    props.dirtyCheck(() => {
      ...
      props.setTargetDepartment(props.sourceDepartment);
...
const dropTargetHOC = DropTarget(
  [DND_DEPARTMENT_TYPE.CARD, DND_ITEM_TYPE.DYNAMIC_LIST_ROW],
  {
    ...
    canDrop(props: IDepartmentCardProps, monitor: DropTargetMonitor) {
      ...
      if (props.activeCard.cardId === props.sourceDepartment.departmentId || !props.isAdmin) {
        return false;
      }
      return true;
    }
