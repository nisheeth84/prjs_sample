...
import SpecialNameField from './special-name-field';
...
const SpecialEditList: React.FC<ISpecialEditListProps> = props => {
  ...
  const [isDisplay, setIsDisplay] = useState(
    props.valueData ? props.valueData[PRODUCT_SPECIAL_FIELD_NAMES.isDisplay] : false
  );
  ...
  const renderComponentIsDisplay = () => {
    if (props.mode === ScreenMode.DISPLAY) {
      ...
      const val = props.valueData[PRODUCT_SPECIAL_FIELD_NAMES.isDisplay]
        ? translate('products.list.label.is-display.true')
        : translate('products.list.label.is-display.false');
      return (
        <>
    ...
    if (props.mode === ScreenMode.DISPLAY) {
      ...
      if (fieldItems && fieldItems.length > 0) {
        const field = fieldItems.find(
          item => item.itemId === props.valueData[PRODUCT_SPECIAL_FIELD_NAMES.productCategoryId]
        );
      ...
      if (props.firstFocus && props.firstFocus.id) {
        if (
          props.firstFocus.id === rowData[props.firstFocus.nameId] &&
          StringUtils.equalPropertyName(props.firstFocus.item, props.itemData.fieldName)
        ) {
          isFocus = true;
        }
      }
  ...
  return (
    <>
      ...
      ].includes(props.itemData.fieldName) && (
        <SpecialNameField
          rowData={props.valueData}
          fieldName={props.itemData.fieldName}
          mode={props.mode}
