...
import ListHeader from './header/custom-list-header';
import ListHeaderContent from './header/custom-list-header-content';
import ListContentRow from './content/custom-list-content-row';
import ListContentCell from './content/custom-list-content-cell';
...
import { MessageType } from 'app/shared/layout/common/box-message';
...
const minWidthByFieldType = {
  ...
  [DEFINE_FIELD_TYPE.SELECT_ORGANIZATION]: 400,
};
...
export interface ICustomDynamicListProp {
  ...
  addFieldUnavailable?: (filed) => void;
  isCanNotDrop?: boolean;
  isCanNotResize?: boolean;
}
...
const CustomDynamicList = forwardRef((props: ICustomDynamicListProp, ref) => {
  ...
  const [originFieldInfoProductSet, setOriginFieldInfoProductSet] = useState(null);

  const keyRecordId = 'productId';
  const fieldNameExtension = 'productTradingData';
  ...
  const onSaveEditField = useCallback(
    (oldFieldInfo, newFieldInfo) => {
      ...
      if (combineFieldInfo.availableFlag === AVAILABLE_FLAG.UNAVAILABLE && props.addFieldUnavailable) {
        props.addFieldUnavailable(combineFieldInfo);
        newFieldInfoProductSet.splice(indexOfField, 1);
      }
  ...
  useEffect(() => {
    if (props.fieldInfoProductSet) {
      ...
      setOriginFieldInfoProductSet(origin);
    }
  }, [props.fieldInfoProductSet]);

  useEffect(() => {
    if (!fieldInfoProductSet) return;
    setTableDynamicWidth(fieldInfoProductSet.map(getColumnWidth).reduce((s, c) => {return s+c;}, 0));
  }, [fieldInfoProductSet]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    resetFieldInfoProductSet() {
      setFieldInfoProductSet(originFieldInfoProductSet);
      setDeletedFieldInfoProductSet([])
    }
  }));

  const totalPrice = useMemo(
    () => (props.products ? props.products.reduce((s, c) => { return s + Number(c.price); }, 0) : 0),
    [props.products]
  );
  ...
  const totalQuantity = useMemo(() => (props.products ? props.products.reduce((s, c) => { return s + Number(c.quantity);}, 0) : 0), [
    props.products
  ]);

  const totalAmount = useMemo(() => (props.products ? props.products.reduce((s, c) => { return s + Number(c.amount);}, 0) : 0), [
    props.products
  ]);
  ...
  const checkValidFieldBelong = (fieldBelong) => {
    return !fieldBelong || fieldBelong === FIELD_BELONG.PRODUCT_TRADING
  }
  ...
  const onDropField = useCallback(
    (fieldInfo, indexToAdd) => {
      ...
      if (indexOfField !== -1) {
        ...
        fieldDrag.fieldBelong = FIELD_BELONG.PRODUCT_TRADING;
  ...
  const onFieldInfoOver = (isOver, dragSource) => {
    if (isOver) {
      if (!dragSource) {
        ...
        const isValidFieldBelong = checkValidFieldBelong(dragItem.fieldBelong)
  ...
  const onDragOverColumnChange = (isOver, dragSource, htmlElement) => {
    ...
    if (isOver) {
      const { value: dragItem } = dragSource;
      isOver = checkValidFieldType(dragItem.fieldType) &&  checkValidFieldBelong(dragItem.fieldBelong) ;
  ...
  /**
   * renderCellHeaderTotal
   * @param fieldInfo
   * @param total
   */
  const renderCellHeaderTotal = (fieldInfo, total) =>{
    return (
      <>
        <div>{getFieldLabel(fieldInfo, 'fieldLabel')}</div>
        <div>
          {translate('products.create-set.th-total')}
          <span className="text-align-right">{total ? StringUtils.numberFormat(total || 0) : 0} {fieldInfo.currencyUnit || ""}</span>
        </div>
      </>
    );
  }
  ...
  const renderCellHeader = (fieldInfo, index: number) => {
    ...
    switch (fieldInfo.fieldName) {
      case 'quantity':
        // allowEdit = false;
        comp = renderCellHeaderTotal(fieldInfo, totalQuantity)
        break;
      case 'price':
        // allowEdit = false;
        comp = renderCellHeaderTotal(fieldInfo, totalPrice)
        break;
      case 'amount':
        // allowEdit = false;
        comp = renderCellHeaderTotal(fieldInfo, totalAmount)
        break;
      case 'unit_price':
        // allowEdit = false;
        comp = (
      ...
      >
        <DropAllowArea
          ...
          onDrop={({ value: dragItem }) => {
            // const indexOfField = fieldInfo.isDefault ? 0 : _.findIndex(fieldInfoProductSet, (i: any) => i.fieldId === fieldInfo.fieldId);
            const indexOfField =  _.findIndex(fieldInfoProductSet, (i: any) => i.fieldId === fieldInfo.fieldId);
            onDropField(dragItem, indexOfField);
          }}
          onIsOverChange={onDragOverColumnChange}
        >
          {props.isCanNotDrop ? (<>{comp}</>) : (<DragItem item={fieldInfo}>{comp}</DragItem>)}
        </DropAllowArea>
        {props.isSetting && allowEdit && !fieldInfo.isDefault && (
          <DeleteColButton
            className="icon-small-primary icon-erase-small"
            onClick={event => deleteField(fieldInfo, event)}
          ></DeleteColButton>
        )}
        {!props.isCanNotResize && <ResizeControl onResize={width => onColResize(fieldInfo, width)} isResize={props.screenMode !== ScreenMode.EDIT}/>}
      </TH>
    );
  };
  ...
  const getTextDataCell = (rowData: any, fieldColumn: any) => {
    ...
    if (_.isUndefined(fieldValue) && !fieldColumn.isDefault && fieldNameExtension) {
      let extData = getValueProp(rowData, fieldNameExtension);
      if (extData) {
        if(isJsonString(extData)){
  ...
  const extractDataDynamic = rowData => {
    if (rowData.productTradingData) {
      if(_.isArray(rowData.productTradingData)){
        rowData.productTradingData.forEach(data => {
          switch (data.key) {
      ...
      } else if(isJsonString(rowData.productTradingData)){
        for (const [key, value] of Object.entries(JSON.parse(rowData.productTradingData))) {
          rowData[key] = value;
        }
      }
    }
  ...
  const customFieldValue = (rowDataCustom, fieldDataCustom, mode) => {
    switch (fieldDataCustom.fieldName.toString()) {
      case 'created_user':
        return ( rowDataCustom.createdUser?.employeeSurname || "" ) + " " + (rowDataCustom.createdUser?.employeeName || "" );
      case 'updated_user':
        return (rowDataCustom.updatedUser?.employeeSurname || "" ) + " " + (rowDataCustom.updatedUser?.employeeName || "" );
      case 'employee':
      case 'employee_id':
        return (rowDataCustom.employee?.employeeSurname || "" ) + " " + (rowDataCustom.employee?.employeeName || "" );
      default:
        return undefined;
    }
  }

  const contentCell = (rowData: any, fieldColumn: any, index: number) => {

    if (isSpecialColumn(fieldColumn) || [PRODUCT_SPECIAL_FIELD_NAMES.createBy, PRODUCT_SPECIAL_FIELD_NAMES.updateBy].includes(fieldColumn.fieldName) ) {
    ...
    const filterErrorByProductId = props.errorItems && _.isArray(props.errorItems) && props.errorItems?.filter(_errorItem => _errorItem.rowId === rowData.productId)

    const findErrorInfo = filterErrorByProductId && _.isArray(filterErrorByProductId) &&  filterErrorByProductId?.find(_errorItem => _errorItem.item === fieldName)
  ...
  return (
    <DivGrid
      className={`wrap-table-scroll table-popup-product style-3 esr-content ${ isDragItemOverOnTable ? 'drag-item-over-table' : ''}`}
    >
