--DO $$
--BEGIN
--    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'relation_sort_value') THEN
			CREATE TYPE relation_sort_value AS (
...
DROP FUNCTION IF EXISTS "calculator_for_sort"("formular" varchar, "target_table" varchar, "column_pk" varchar, "target_id" int8);

DROP FUNCTION IF EXISTS "get_relation_value_for_sort"(character varying,character varying,character varying,character varying,jsonb,character varying);
