...
BEGIN
	SELECT fir.field_name, fir.field_type, fir.field_belong INTO field_name_var, field_type_var, field_belong_var
		...
		IF field_belong_var = 15
		AND ( field_name_var LIKE 'finish_date' 
		OR field_name_var LIKE 'start_date' ) THEN
			BEGIN
				sql_var = format('SELECT calendars_view.%s FROM tasks_view INNER JOIN calendars_view ON tasks_view.calendar_id = calendars_view.calendar_id WHERE tasks_view.%I = %s'				, field_name_var, column_pk, ids->>0);
