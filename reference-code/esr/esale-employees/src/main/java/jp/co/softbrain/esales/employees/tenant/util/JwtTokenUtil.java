...
public class JwtTokenUtil implements Serializable {
    ...
    public String getLanguageCodeFromToken() {
        ...
        if (StringUtils.isBlank(languageCode)) {
            languageCode = (String) getTokenInfo(Constants.Authenticate.LANGUAGE_CODE);
        }
        return languageCode;
    }
    ...
    public String getFormatDateFromToken() {
        String formatDate = request.getHeader("formatDate");
        if (StringUtils.isBlank(formatDate)) {
            formatDate = (String) getTokenInfo(Constants.Authenticate.FORMAT_DATE);
        }
        return formatDate;
    }

    /**
     * get time zone from token
     *
     * @return
     */
    public String getTimeZoneFromToken() {
        ...
        if (StringUtils.isBlank(timezone)) {
            timezone = (String) getTokenInfo(Constants.Authenticate.TIMEZONE);
        }
        return timezone;
    }
    
    /**
     * get flag access contract site
     *
     * @return
     */
    public Boolean getIsAccessContract() {
        Object isAccess = getTokenInfo(Constants.Authenticate.IS_ACCESS_CONTRACT);
        return isAccess != null && Boolean.parseBoolean(isAccess.toString());
    }
