...
function month(xd) {
  const year = xd.getFullYear(), month_ = xd.getMonth();
  const days = new Date(year, month_ + 1, 0).getDate();

  const firstDay = new XDate(year, month_, 1, 0, 0, 0, true);
  const lastDay = new XDate(year, month_, days, 0, 0, 0, true);
