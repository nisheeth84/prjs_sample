...
import { TypeButton, STATUSBUTTON, LanguageCode } from '../../../../config/constants/enum';
...
export const ListTasks = () => {
  ...
  */
  ...
  const handleEditTask = (task: TaskItem) => {
    navigation.navigate("add-task", { task, type: 2 });
  }
   ...
   * @param statusTaskId status update
        ...
        >
          <View style={ListTaskStyle.statusAndCustomerContainer} >
            <View style={ListTaskStyle.customerNameContainer}>
              ...
              {item.customers[0] ? 
                `${item.customers[0]?.customerName} / ${item.productTradings.map((product) => product.productTradingName).join(',')}`
                : ''
              }
              </Text>
            </View>
          ...
          <View style={ListTaskStyle.infoContainer}>
            <View style={ListTaskStyle.iconContainer}>
              {item.milestoneId ?
                ...
                <View style={[ListTaskStyle.iconItem, ListTaskStyle.fakeIconMileStone]}></View>
              }
              {item?.subtasks?.length > 0 ?
                <Icon name={'memo'} style={ListTaskStyle.iconItem} /> :
                <View style={[ListTaskStyle.iconItem, ListTaskStyle.fakeIconSubtask]}></View>
              }
              <TouchableOpacity
