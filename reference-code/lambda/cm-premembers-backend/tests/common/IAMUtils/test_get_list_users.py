...
@mock_sts
@mock_iam
class TestGetListUsers(TestCaseBase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_list_users_success_response_is_truncate_false(self):
        # connect client
        client_connect_iam = iam_utils.client_connect()

        expected_list_users = copy.deepcopy(
            DataTestIAM.DATA_LIST_USER_IS_TRUNCATED_FALSE)
        ...
        with patch.object(session, 'client') as mock_method_client:
            ...
            with patch.object(client_connect_iam, 'list_users') as mock_method:
                mock_method.return_value = expected_list_users
                actual_list_users = IAMUtils.get_list_users(
                    trace_id, session, aws_account)

        # check response
        self.assertEqual(expected_list_users['Users'], actual_list_users)
    ...
    def test_get_list_users_error_call_list_users(self):
        # connect client
        client_connect_iam = iam_utils.client_connect()
        ...
        # check message log error
        mock_method_error.assert_any_call("[%s] IAMユーザー一覧情報の取得に失敗しました。",
                                          aws_account)
