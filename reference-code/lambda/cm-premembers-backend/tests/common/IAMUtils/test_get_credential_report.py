...
@mock_sts
@mock_iam
class TestGetCredentialReport(TestCaseBase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_credential_report_error_connect_iam(self):
        ...
        with patch.object(session, 'client') as mock_method:
            ...
            with patch.object(
                ...
                with self.assertRaises(PmError) as exception:
                    IAMUtils.get_credential_report(trace_id, session,
                                                   aws_account)
    ...
    def test_get_credential_report_error_call_get_credential_report(self):
        # connect client
        client_connect_iam = iam_utils.client_connect()
        ...
        with patch.object(session, 'client') as mock_client:
            ...
            # mock error call API get_credential_report
            with patch.object(client_connect_iam,
                              'get_credential_report') as mock_method:
                mock_method.side_effect = ClientError(
                    error_response=expected_error_response,
        ...
        # check message log error
        mock_method_error.assert_any_call("[%s] CredentialReportの取得に失敗しました。",
                                          aws_account)
