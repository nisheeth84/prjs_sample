...
@mock_sts
@mock_iam
class TestGetAccountPasswordPolicy(TestCaseBase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_account_password_policy_success_response_exists_password_policy(self):
        # connect client
    ...
    def test_get_account_password_policy_success_response_not_exists_password_policy(self):
        # connect client
        client_connect_iam = iam_utils.client_connect()
    ...
    def test_get_account_password_policy_error_other_no_such_entity(self):
        # connect client
        client_connect_iam = iam_utils.client_connect()
        ...
        with patch.object(session, 'client') as mock_method_client:
            mock_method_client.return_value = client_connect_iam
            # mock error call API get_account_password_policy
            with patch.object(client_connect_iam,
                              'get_account_password_policy') as mock_method:
                mock_method.side_effect = ClientError(
                    error_response=expected_error_response,
        ...
        # check message log error
        mock_method_error.assert_any_call("[%s] アカウントパスワードポリシー情報の取得に失敗しました。",
                                          aws_account)
