...
from tests.mock.data.aws.dynamodb.data_pm_assessment_items import DataPmAssessmentItems
...
data_pm_assessment_items = copy.deepcopy(DataPmAssessmentItems.DATA_SIMPLE)

path_parameters = {
    "organization_id": data_pm_assessment_items['OrganizationID'],
    "project_id": data_pm_assessment_items['ProjectID'],
    "coop_id": data_pm_aws_account_coops['CoopID'],
    "check_item_code": data_pm_assessment_items['CheckItemCode']
}
...
@mock_dynamodb2
class TestGetAssessmentItemHandler(TestCaseBase):
    def setUp(self):
        super().setUp()
    ...
    def test_get_assessment_item_handler_case_assessment_items_zero_record(self):
        # perpare data test
        mock_pm_affiliation.create(data_pm_affiliation)
        mock_pm_awsAccountCoops.create(data_pm_aws_account_coops)

        # Call function test
        actual_response = checkitemsettings.get_assessment_item_handler(
            event_mock, {})
