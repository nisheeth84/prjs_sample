...
//VietNT_20190527_start
    public List<StockDTO> getForAutoCompleteStockDomain(StockDTO obj) {
        String sql = "SELECT ST.CAT_STOCK_ID catStockId" + " ,ST.NAME name" + " ,ST.CODE code" + " FROM CAT_STOCK ST"
                + " WHERE ST.STATUS=1 ";
...
//        if (StringUtils.isNotEmpty(obj.getStockType())) {//truong hop tim kiem kho tam
//            stringBuilder.append(" AND STOCK_TYPE = :stockType ");
//        } else {//neue khong lay cac kho co stock type != 3
//            stringBuilder.append(" AND STOCK_TYPE != 3 ");
//        }
//        if(!StringUtils.isNotEmpty(obj.getStockType())){// kho tam khong check phan quyen
//            if (obj.getListId().size() > 0) {
//                stringBuilder.append(" AND ST.CAT_STOCK_ID in (:listId)");
//            } else {
//                stringBuilder.append(" AND ST.CAT_STOCK_ID is null ");
//            }
//        }
//        /**Hoangnh start 01032019**/
//        if(obj.getSysGroupId() != null){
//            stringBuilder.append(" AND SYS_GROUP_ID =:sysGroupId ");
//        }
//        /**Hoangnh end 01032019**/
...
//        if (obj.getListId().size() > 0 && !StringUtils.isNotEmpty(obj.getStockType())) {
...
//VietNT_end
    ...
    public List<StockDTO> doSearchStockInPopUpDomain(StockDTO obj) {
        String sql = "SELECT ST.CAT_STOCK_ID catStockId, " + "ST.NAME name," + "ST.CODE code," + "dp.NAME sysGroupName "
                + "FROM CAT_STOCK ST " + "LEFT JOIN SYS_GROUP dp ON ST.SYS_GROUP_ID = dp.SYS_GROUP_ID "
                + "WHERE ST.STATUS=1 ";
        StringBuilder stringBuilder = new StringBuilder(sql);
        if (StringUtils.isNotEmpty(obj.getName())) {
            stringBuilder.append(
...
//            stringBuilder.append(" AND STOCK_TYPE = :stockType ");
//        }
//        if (!StringUtils.isNotEmpty(obj.getStockType())) { //kho tam k kiem tra phan quyen
//            if (obj.getListId().size() > 0) {
//                stringBuilder.append(" AND ST.CAT_STOCK_ID in (:listId) ");
//            } else {
//                stringBuilder.append(" AND ST.CAT_STOCK_ID is null ");
