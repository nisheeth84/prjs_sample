...
@Service("aioQuestionBusiness")
@Scope(proxyMode = ScopedProxyMode.TARGET_CLASS)
public class AIOQuestionBusinessImpl extends BaseFWBusinessImpl<AIOQuestionDAO, AIOQuestionDTO, AIOQuestionBO> implements AIOQuestionBusiness {

    @Autowired
    private AIOQuestionDAO aioQuestionDAO;

    protected final Logger log = Logger.getLogger(AIOQuestionBusinessImpl.class);
    ...
    @Autowired
    public AIOQuestionBusinessImpl(AIOQuestionDAO aioQuestionDAO) {
            ...
            for (Row row : sheet) {
                ...
                if (StringUtils.isNotEmpty(type)) {
                    boolean isInput = AIOQuestionDTO.TYPE_INPUT_NAME.toUpperCase().equals(type.toUpperCase().trim())
                            || AIOQuestionDTO.TYPE_INPUT.toString().equals(type.toUpperCase().trim());
                    boolean isSelect = AIOQuestionDTO.TYPE_SELECT_NAME.toUpperCase().equals(type.toUpperCase().trim())
                            || AIOQuestionDTO.TYPE_SELECT.toString().equals(type.toUpperCase().trim());
                    if (!isInput && !isSelect) {
