...
import com.viettel.aio.dao.AIORpRequestNOKDAO;
...
//StephenTrung__3_12_2019_created
@Service("AIORpRequestNOKBusinessImpl")
@Scope(proxyMode = ScopedProxyMode.TARGET_CLASS)
public class AIORpRequestNOKBusinessImpl implements AIORpRequestNOKBusiness {

    static Logger LOGGER = LoggerFactory.getLogger(AIORpRequestNOKBusinessImpl.class);

    @Autowired
    private AIORpRequestNOKDAO aioRpRequestNOKDAO;
    ...
    public DataListDTO doSearch(AIOOrdersDTO criteria) {
        ...
        if (criteria.getIsDetail() == 0){
            dtos = aioRpRequestNOKDAO.doSearch(criteria);
        }else if (criteria.getIsDetail() == 1){
            dtos = aioRpRequestNOKDAO.doSearchDetail(criteria);
        }
    ...
    private final String RP_REQUEST_NOK = "Bao_cao_khao_sat_cong_viec_NOK.xlsx";

    @Override
    public String exportExcel(AIOOrdersDTO criteria) throws Exception {
        List<AIOOrdersDTO> dtos = aioRpRequestNOKDAO.doSearch(criteria);
